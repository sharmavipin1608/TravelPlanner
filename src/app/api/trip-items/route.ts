  ...
  try {
    ...
    if (!user) {
      return Response.json(
        { data: null, error: { code: 'unauthorized', message: 'Not authenticated' } } satisfies ApiResponse<string[]>,
        { status: 401 }
      )
    }
    ...
    if (!trip_id) {
      return Response.json(
        { data: null, error: { code: 'validation_error', message: 'trip_id is required' } } satisfies ApiResponse<string[]>,
        { status: 400 }
      )
    }
    ...
    if (!trip) {
      return Response.json(
        { data: null, error: { code: 'not_found', message: 'Trip not found' } } satisfies ApiResponse<string[]>,
        { status: 404 }
      )
    }
    ...
    if (error) {
      return Response.json(
        { data: null, error: { code: 'db_error', message: 'Internal server error' } } satisfies ApiResponse<string[]>,
        { status: 500 }
      )
    }

    return Response.json({ data: data.map((r) => r.item_id) as string[], error: null } satisfies ApiResponse<string[]>)
  } catch {
    return Response.json(
      { data: null, error: { code: 'internal', message: 'Internal server error' } } satisfies ApiResponse<string[]>,
      { status: 500 }
    )
  }
}
